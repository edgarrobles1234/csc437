...
export class EdibleCardElement extends HTMLElement {
      ...
      <article class="card">
        ...
        <div class="details">
          <p>
            ...
            <slot name="scientific-name">Unknown</slot>
          </p>
          <p>
            <strong>Found In:</strong>
            <slot name="found-in">Unknown</slot>
          </p>
          <p>
            <strong>Safe to Eat:</strong>
            <slot name="safe-to-eat">Unknown</slot>
          </p>
          <p>
            <strong>Harvest Notes:</strong>
            <slot name="harvest-notes">None</slot>
          </p>
          <p>
            <strong>Sustainability Notes:</strong>
            <slot name="sustainability-notes">None</slot>
          </p>
        </div>
  ...
  constructor() {
    ...
    shadow(this)
      ...
      .styles(EdibleCardElement.styles);
  }
  ...
  attributeChangedCallback(name: string, _oldValue: string | null, newValue: string | null) {
    ...
    if (name === "wiki-href") {
      const link = this.shadowRoot.querySelector<HTMLAnchorElement>("#wiki-link");
      if (link) link.setAttribute("href", newValue || "#");
    }

    if (name === "icon") {
      const use = this.shadowRoot.querySelector<SVGUseElement>("#icon-use");
      if (!use) return;
